...
from app.services.ai import summarize_text, describe_image, transcribe_voice, extract_reminder_from_text, embed_text
...
async def list_notes(db: AsyncSession = Depends(get_db)):
    res = await db.execute(select(Note).order_by(Note.created_at.desc()))
...
@router.get('/chat')
async def chat_with_notes(q: str, db: AsyncSession = Depends(get_db)):
    ids = vector_store.semantic_search(q)
    if not ids:
